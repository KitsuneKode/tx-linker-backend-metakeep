...
app.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    console.log(
      `[${new Date().toISOString()}] GET /api/analytics/event - Request received`,
    );

    try {
      console.log(`[${new Date().toISOString()}] Querying top 50 events`);
      ...
      console.log(`[${new Date().toISOString()}] Event query result:`, result);
      ...
      console.log(`[${new Date().toISOString()}] Returning top events data`);
